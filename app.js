...
function navigateTo(viewId) {
  ...
  // Handle Three.js scene animation states
  if (viewId === 'view-home') {
    startThreeJSScene();
  } else {
    stopThreeJSScene();
...
// =============================================
// --- THREE.JS GENERATIVE ART SCENE CONTROLLER ---
// =============================================
...
function startThreeJSScene() {
  ...
  if (threeSceneInstance) {
    // Already running or initialized. If paused, resume.
    threeSceneInstance.paused = false;
    return;
  }
  ...
  const animate = (t) => {
    ...
    if (!threeSceneInstance.paused) {
      material.uniforms.time.value = t * 0.0003;
      mesh.rotation.y += 0.0005;
      mesh.rotation.x += 0.0002;
      renderer.render(scene, camera);
    }
    threeSceneInstance.frameId = requestAnimationFrame(animate);
  };
  animate(0);

  const handleResize = () => {
    if (!threeSceneInstance || !container) return;
  ...
  const handleMouseMove = (e) => {
    if (!threeSceneInstance || threeSceneInstance.paused) return;
    ...
    threeSceneInstance.pointLight.position.copy(pos);
  ...
  threeSceneInstance.cleanup = () => {
    if (threeSceneInstance.frameId) {
      cancelAnimationFrame(threeSceneInstance.frameId);
    }
...
function stopThreeJSScene() {
  if (threeSceneInstance) {
    threeSceneInstance.paused = true;
  }
}

window.startThreeJSScene = startThreeJSScene;
window.stopThreeJSScene = stopThreeJSScene;

// =============================================
// --- PUBLIC NAVIGATION ROUTING HELPERS ---
// =============================================
function goHome() {
  if (!State.currentUser || !State.currentProfile) {
    navigateTo('view-home');
...
function scrollToFeatures(e) {
  e.preventDefault();
  navigateTo('view-home');
  // Delay slightly to allow display transitions to trigger scroll offsets
  setTimeout(() => {
